                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {shifts?.filter(s => s.type !== 'parttime').map((shift) => {
                                const slots = formData.date ? getShiftSlots(shift.id) : null;
                                const isDisabled = slots ? slots.available === 0 : false;
                                const isSelected = formData.shiftId === shift.id;

                                return (
                                    <div
                                        key={shift.id}
                                        onClick={() => !isDisabled && handleShiftChange(shift.id)}
                            ...
                            {shifts?.find(s => s.type === 'parttime') && (
                                <div
                                    onClick={() => handleShiftChange(shifts.find(s => s.type === 'parttime')?.id!)}
                                    className={`relative p-4 rounded-xl border-2 transition-all cursor-pointer group flex flex-col justify-between min-h-[120px] ${formData.shiftId === shifts.find(s => s.type === 'parttime')?.id
                                ...
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        <h3 className={`font-bold text-lg ${formData.shiftId === shifts.find(s => s.type === 'parttime')?.id ? 'text-purple-700 dark:text-purple-300' : 'text-gray-900 dark:text-white'}`}>
                                            Part-time
                                        </h3>
                                        {formData.shiftId === shifts.find(s => s.type === 'parttime')?.id && (
